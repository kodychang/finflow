...
enum DocumentPDFExporter {
    ...
    private static func measuredContentHeight(for document: BusinessDocument, in rect: CGRect, language: AppLanguage) -> CGFloat {
        ...
        let paymentHeight = document.type.showsPaymentDetails
            ? measuredTextHeight(document.paymentDetails.isEmpty ? "-" : document.paymentDetails, width: 280, font: pdfFont(10.5), minimum: 24)
            : 0
        ...
        let paymentProofHeight = document.type.isVendorForm
            ...
            : 0
        let summaryHeight = max(170, 82 + notesHeight + (paymentHeight > 0 ? paymentHeight + 32 : 0) + memoHeight + paymentProofHeight)
        let amountBoxHeight: CGFloat = (document.type.showsSummaryTotals || document.type.showsPaymentDetails) ? 64 : 0
        let tableStartY = rect.minY + 84 + 17 + companyInfoHeight(for: document) + 14 + amountBoxHeight + (amountBoxHeight > 0 ? 18 : 0)
    ...
    private static func companyInfoHeight(for document: BusinessDocument) -> CGFloat {
        ...
        let showsRegistration = document.type.showsIssuerRegistration && !document.issuerRegistration.isEmpty
        let issuerNameY: CGFloat = showsRegistration ? 38 : 0
    ...
    private static func draw(_ document: BusinessDocument, in rect: CGRect, language: AppLanguage) {
        ...
        let showsDueDate = document.type.showsDueDate
        let showsLinePrices = document.type.showsLinePrices
        let showsSummaryTotals = document.type.showsSummaryTotals
        let showsTax = document.type.showsTax
        let showsPaymentDetails = document.type.showsPaymentDetails
        let showsIssuerRegistration = document.type.showsIssuerRegistration && !document.issuerRegistration.isEmpty
        let displayTotal = showsTax ? document.total : document.subtotal
        ...
        if showsDueDate {
            metaItems.append((pdfLabel(.dueDate, language), pdfDate(document.dueDate, language: language)))
        }
        metaItems.append((pdfLabel(.relatedNumber, language), document.relatedNumber.isEmpty ? pdfLabel(.noRelatedNumber, language) : document.relatedNumber))
        metaItems.enumerated().forEach { index, item in
        ...
        let issuerNameY = showsIssuerRegistration ? y + 38 : y
        ...
        let issuerBottom = max(showsIssuerRegistration ? y + 40 : issuerNameY + 18, issuerContactY + issuerContactHeight)

        y = max(customerBottom, issuerBottom) + 14
        if showsSummaryTotals || showsPaymentDetails {
            ...
            if showsSummaryTotals {
                ...
                drawRightText(AppFormatters.yen(displayTotal, language: language), in: CGRect(x: amountBox.maxX - 150, y: amountBox.minY + 5, width: 140, height: 22), attributes: totalAttributes)
            }
            if showsPaymentDetails {
                let paymentY = showsSummaryTotals ? amountBox.minY + 42 : amountBox.minY + 12
                drawText(pdfLabel(.paymentAndDueDate, language), at: CGPoint(x: amountBox.minX + 10, y: paymentY), width: 160, height: 13, attributes: boldAttributes)
                drawRightTextToFit(
                    singleLinePaymentText(document.paymentDetails),
                    in: CGRect(x: amountBox.minX + 176, y: paymentY - 8, width: amountBox.width - 186, height: 12),
                    attributes: bodyAttributes,
                    minimumScale: 0.58
                )
                if showsDueDate {
                    drawRightText("\(pdfLabel(.dueDate, language)): \(pdfDate(document.dueDate, language: language))", in: CGRect(x: amountBox.maxX - 220, y: paymentY + 6, width: 210, height: 12), attributes: bodyAttributes)
                }
            }
            y = amountBox.maxY + 18
        }
        let widths: [CGFloat] = showsLinePrices ? [150, 145, 50, 76, 90] : [210, 240, 61]
        let headers = showsLinePrices
            ? [pdfLabel(.item, language), pdfLabel(.modelSpec, language), pdfLabel(.quantity, language), pdfLabel(.unitPrice, language), pdfLabel(.amount, language)]
            : [pdfLabel(.item, language), pdfLabel(.modelSpec, language), pdfLabel(.quantity, language)]
        drawTableHeader(headers, y: y, x: rect.minX, widths: widths, attributes: boldAttributes, fill: template.tableHead)
        y += 23
        for line in document.lines {
            let row = showsLinePrices ? [
            ...
            drawInvoiceTableRow(row, y: y, x: rect.minX, widths: widths, attributes: bodyAttributes, stroke: template.softLine)
            y += 34
        }
        ...
        var memoY = y + 18 + notesHeight + 14
        if showsPaymentDetails {
            drawText(pdfLabel(.paymentDetails, language), at: CGPoint(x: memoX, y: memoY), width: 160, height: 18, attributes: [
                .font: pdfFont(12, weight: .medium),
                .foregroundColor: UIColor.black,
            ])
            ...
            drawMultiline(document.paymentDetails.isEmpty ? "-" : document.paymentDetails, in: CGRect(x: memoX, y: memoY + 18, width: 280, height: paymentHeight), attributes: bodyAttributes)
            memoY += 18 + paymentHeight + 14
        }
        ...
        if showsSummaryTotals {
            ...
            let totalLineY = showsTax ? y + 38 : y + 20
            let totalTextY = showsTax ? y + 52 : y + 34
            drawLine(from: CGPoint(x: totalX, y: totalLineY), to: CGPoint(x: totalX + 190, y: totalLineY), color: template.accent, lineWidth: 1.6)
            drawText(pdfLabel(.total, language), at: CGPoint(x: totalX, y: totalTextY), width: 70, height: 22, attributes: [
                .font: pdfFont(16, weight: .medium),
                .foregroundColor: UIColor.black,
            ])
            drawRightText(AppFormatters.yen(displayTotal, language: language), in: CGRect(x: totalX + 72, y: totalTextY - 3, width: 118, height: 27), attributes: totalAttributes)
        }
    }
    ...
    private static func openingSentence(for type: DocumentType, language: AppLanguage) -> String {
        ...
        case (let type, .traditionalChinese):
            return openingSentence(for: type, language: .simplifiedChinese)
        case (.invoice, .japanese): return "下記の通り、ご請求申し上げます。"
        case (.invoice, .simplifiedChinese): return "现按以下内容向贵司请款。"
        case (.invoice, .english), (.invoice, .korean), (.invoice, .nepali), (.invoice, .french), (.invoice, .vietnamese): return "Please find the invoice details below."
        case (.estimate, .japanese): return "下記の通り、お見積り申し上げます。"
        case (.estimate, .simplifiedChinese): return "现按以下内容报价。"
        case (.estimate, .english), (.estimate, .korean), (.estimate, .nepali), (.estimate, .french), (.estimate, .vietnamese): return "Please find the quote details below."
        case (.delivery, .japanese): return "下記の通り、納品いたします。"
        case (.delivery, .simplifiedChinese): return "现按以下内容送货。"
        case (.delivery, .english), (.delivery, .korean), (.delivery, .nepali), (.delivery, .french), (.delivery, .vietnamese): return "The following items are delivered."
        case (.receipt, .japanese): return "下記の金額を領収いたしました。"
        case (.receipt, .simplifiedChinese): return "已收到以下款项。"
        case (.receipt, .english), (.receipt, .korean), (.receipt, .nepali), (.receipt, .french), (.receipt, .vietnamese): return "Payment has been received as detailed below."
        case (.acceptance, .japanese): return "下記の通り、受領いたしました。"
        case (.acceptance, .simplifiedChinese): return "已按以下内容收货。"
        case (.acceptance, .english), (.acceptance, .korean), (.acceptance, .nepali), (.acceptance, .french), (.acceptance, .vietnamese): return "The following items have been accepted."
        case (.purchaseOrder, .japanese): return "下記の通り、発注いたします。"
        case (.purchaseOrder, .simplifiedChinese): return "现按以下内容下单采购。"
        case (.purchaseOrder, .english), (.purchaseOrder, .korean), (.purchaseOrder, .nepali), (.purchaseOrder, .french), (.purchaseOrder, .vietnamese): return "Please process the purchase order below."
        case (.customerOrder, .japanese): return "下記の通り、受注内容を記録します。"
        case (.customerOrder, .simplifiedChinese): return "受注内容记录如下。"
        case (.customerOrder, .english), (.customerOrder, .korean), (.customerOrder, .nepali), (.customerOrder, .french), (.customerOrder, .vietnamese): return "Order received details are recorded below."
        case (.customerFiles, .japanese): return "下記の通り、顧客書類を管理します。"
        case (.customerFiles, .simplifiedChinese): return "客户文件管理内容如下。"
        case (.customerFiles, .english), (.customerFiles, .korean), (.customerFiles, .nepali), (.customerFiles, .french), (.customerFiles, .vietnamese): return "Project document details are listed below."
        case (.vendorEstimate, .japanese): return "下記の通り、仕入先見積内容を記録します。"
        case (.vendorEstimate, .simplifiedChinese): return "厂商报价内容记录如下。"
        case (.vendorEstimate, .english), (.vendorEstimate, .korean), (.vendorEstimate, .nepali), (.vendorEstimate, .french), (.vendorEstimate, .vietnamese): return "Vendor quotation details are recorded below."
        case (.vendorInvoice, .japanese): return "下記の通り、仕入先請求書内容を記録します。"
        case (.vendorInvoice, .simplifiedChinese): return "厂商请款书内容记录如下。"
        case (.vendorInvoice, .english), (.vendorInvoice, .korean), (.vendorInvoice, .nepali), (.vendorInvoice, .french), (.vendorInvoice, .vietnamese): return "Vendor invoice details are recorded below."
        case (.vendorReceipt, .japanese): return "下記の通り、仕入先領収書内容を記録します。"
        case (.vendorReceipt, .simplifiedChinese): return "厂商收据内容记录如下。"
        case (.vendorReceipt, .english), (.vendorReceipt, .korean), (.vendorReceipt, .nepali), (.vendorReceipt, .french), (.vendorReceipt, .vietnamese): return "Vendor receipt details are recorded below."
        case (.paymentNotice, .japanese): return "下記の通り、支払通知内容を記録します。"
        case (.paymentNotice, .simplifiedChinese): return "支付告知通知内容记录如下。"
        case (.paymentNotice, .english), (.paymentNotice, .korean), (.paymentNotice, .nepali), (.paymentNotice, .french), (.paymentNotice, .vietnamese): return "Payment notice details are recorded below."
        }
    }
    ...
    private static func defaultCondition(for type: DocumentType, language: AppLanguage) -> String {
        switch (type, language) {
        case (let type, .traditionalChinese):
            return defaultCondition(for: type, language: .simplifiedChinese)
        case (.invoice, .japanese): return "振込先、支払期限、請求条件を確認してください。"
        case (.invoice, .simplifiedChinese): return "请确认收款账户、付款期限和付款条件。"
        case (.invoice, .english), (.invoice, .korean), (.invoice, .nepali), (.invoice, .french), (.invoice, .vietnamese): return "Please confirm payment details, due date, and invoice terms."
        case (.estimate, .japanese): return "見積条件、納入予定、税率・合計金額を確認してください。"
        case (.estimate, .simplifiedChinese): return "请确认报价条件、交付计划、税率和合计金额。"
        case (.estimate, .english), (.estimate, .korean), (.estimate, .nepali), (.estimate, .french), (.estimate, .vietnamese): return "Please confirm quote terms, delivery schedule, tax rate, and total."
        case (_, .japanese): return "内容、期限、条件を確認してください。"
        case (_, .simplifiedChinese): return "请确认内容、期限和条件。"
        case (_, .english), (_, .korean), (_, .nepali), (_, .french), (_, .vietnamese): return "Please confirm the details, deadlines, and terms."
        }
    }

    private static func pdfLabel(_ key: PDFLabel, _ language: AppLanguage) -> String {
        switch (key, language) {
        case (let key, .traditionalChinese):
            return pdfLabel(key, .simplifiedChinese)
        case (.number, .japanese): return "番号"
        case (.number, .simplifiedChinese): return "编号"
        case (.number, .english), (.number, .korean), (.number, .nepali), (.number, .french), (.number, .vietnamese): return "No."
        ...
        case (.dueDate, .english), (.dueDate, .korean), (.dueDate, .nepali), (.dueDate, .french), (.dueDate, .vietnamese): return "Due Date"
        ...
        case (.noRelatedNumber, .english), (.noRelatedNumber, .korean), (.noRelatedNumber, .nepali), (.noRelatedNumber, .french), (.noRelatedNumber, .vietnamese): return "No reference"
        case (.customerName, .japanese): return "取引先名"
        case (.customerName, .simplifiedChinese): return "客户名称"
        case (.customerName, .english), (.customerName, .korean), (.customerName, .nepali), (.customerName, .french), (.customerName, .vietnamese): return "Customer Name"
        ...
        case (.qualifiedInvoice, .english), (.qualifiedInvoice, .korean), (.qualifiedInvoice, .nepali), (.qualifiedInvoice, .french), (.qualifiedInvoice, .vietnamese): return "Qualified Invoice"
        ...
        case (.paymentAndDueDate, .english), (.paymentAndDueDate, .korean), (.paymentAndDueDate, .nepali), (.paymentAndDueDate, .french), (.paymentAndDueDate, .vietnamese): return "Payment / Due Date"
        case (.item, .japanese): return "品目"
        case (.item, .simplifiedChinese): return "项目"
        case (.item, .english), (.item, .korean), (.item, .nepali), (.item, .french), (.item, .vietnamese): return "Item"
        case (.modelSpec, .japanese): return "型番 / 仕様"
        case (.modelSpec, .simplifiedChinese): return "型号 / 规格"
        case (.modelSpec, .english), (.modelSpec, .korean), (.modelSpec, .nepali), (.modelSpec, .french), (.modelSpec, .vietnamese): return "Model / Specs"
        case (.quantity, .japanese): return "数量"
        case (.quantity, .simplifiedChinese): return "数量"
        case (.quantity, .english), (.quantity, .korean), (.quantity, .nepali), (.quantity, .french), (.quantity, .vietnamese): return "Qty"
        ...
        case (.paymentDetails, .english), (.paymentDetails, .korean), (.paymentDetails, .nepali), (.paymentDetails, .french), (.paymentDetails, .vietnamese): return "Payment Details"
        case (.paymentProof, .japanese): return "支払証明"
        case (.paymentProof, .simplifiedChinese): return "支付证明"
        case (.paymentProof, .english), (.paymentProof, .korean), (.paymentProof, .nepali), (.paymentProof, .french), (.paymentProof, .vietnamese): return "Payment Proof"
        case (.paymentProofDate, .japanese): return "支払日時"
        case (.paymentProofDate, .simplifiedChinese): return "支付时间"
        case (.paymentProofDate, .english), (.paymentProofDate, .korean), (.paymentProofDate, .nepali), (.paymentProofDate, .french), (.paymentProofDate, .vietnamese): return "Payment Time"
        case (.paymentProofAmount, .japanese): return "支払金額"
        case (.paymentProofAmount, .simplifiedChinese): return "支付金额"
        case (.paymentProofAmount, .english), (.paymentProofAmount, .korean), (.paymentProofAmount, .nepali), (.paymentProofAmount, .french), (.paymentProofAmount, .vietnamese): return "Payment Amount"
        case (.paymentProofFiles, .japanese): return "添付ファイル"
        case (.paymentProofFiles, .simplifiedChinese): return "附件"
        case (.paymentProofFiles, .english), (.paymentProofFiles, .korean), (.paymentProofFiles, .nepali), (.paymentProofFiles, .french), (.paymentProofFiles, .vietnamese): return "Files"
        case (.invoiceTerms, .japanese): return "請求条件"
        case (.invoiceTerms, .simplifiedChinese): return "付款条件"
        case (.invoiceTerms, .english), (.invoiceTerms, .korean), (.invoiceTerms, .nepali), (.invoiceTerms, .french), (.invoiceTerms, .vietnamese): return "Invoice Terms"
        case (.terms, .japanese): return "条件"
        case (.terms, .simplifiedChinese): return "条件"
        case (.terms, .english), (.terms, .korean), (.terms, .nepali), (.terms, .french), (.terms, .vietnamese): return "Terms"
        case (.subtotal, .japanese): return "小計"
        case (.subtotal, .simplifiedChinese): return "小计"
        case (.subtotal, .english), (.subtotal, .korean), (.subtotal, .nepali), (.subtotal, .french), (.subtotal, .vietnamese): return "Subtotal"
        case (.tax, .japanese): return "消費税"
        case (.tax, .simplifiedChinese): return "消费税"
        case (.tax, .english), (.tax, .korean), (.tax, .nepali), (.tax, .french), (.tax, .vietnamese): return "Tax"
        case (.total, .japanese): return "合計"
        case (.total, .simplifiedChinese): return "合计"
        case (.total, .english), (.total, .korean), (.total, .nepali), (.total, .french), (.total, .vietnamese): return "Total"
        }
    }
